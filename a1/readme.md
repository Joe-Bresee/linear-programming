...
*Author: Joseph Bresee V01005288*
...
Karmarkar (1984) introduced the first practical polynomial-time LP algorithm, requiring $O(n^{3.5}L)$ operations (which before Karmarkar was deemed computationally infeasible). Rather than traversing the boundary of the feasible polytope, which is how Simplex works, Karmarkar's method moves through the interior of the polytope through a sequence of projective transformations. This algorithm is used in my benchmark test as the HiGHS IPM backend, standing as a non-Simplex, performative, competitive option for solving LPs.

### 2.3 Smoothed Analysis: Spielman and Teng
Spielman and Teng (2004) gave theoretical understanding behind Simplex's observed polynomial behaviour in practice while Klee-Minty's exponential worst case polytope existed [3]. Their framework of smoothed analysis perturbs an input with small Gaussian noise. Their result is that the expected number of pivot steps on any perturbed instance is bounded by a polynomial, demonstrating an expected polynomial time.

The insight gathered from the Klee-Minty cube and smoothed analysis is that while the Simplex method has a technically worst-case exponential time, this only occurs in very specific polytope shapes. This in combination with common real-world factors like floating-point rounding and measurement errors causes Klee-Minty cube style polytopes to be highly unlikely in practice, leading to consistent polynomial behaviour from Simplex empirically. Smoothed analysis proved to demonstrate that the exponential worst-case polytopes are fragile and break down to polynomial worst-case when perturbed [3].

### 2.4 Algorithmic Engineering: Huangfu and Hall
Modern industrial-grade Simplex solvers differ dramatically from the textbook description. Huangfu and Hall (2018) describe the design of the HiGHS dual revised simplex solver, which incorporates parallelism across multiple pivots (the PAMI strategy) and other improvements [4]. Their computational results show HiGHS simplex outperforming earlier leading open-source simplex implementations and achieving competitive performance with IPM on standard LP benchmark sets. The design of the HiGHS simplex solver demonstrates that there is a significant gap between simple implementations of an algorithm and a production-level solver. This insight is highlighted in my personal experiment results below.

## 3 LP Model and Problem Formulation

The benchmark is a resource-allocation scheduling LP with $n$ jobs and $m=10$ resources. For job $i\in\{1,...,n\}$ and resource $r\in\{1,...,m\}$, the decision variable $x_{i,r}\in[0,1]$ is the fraction of resource $r$ allocated to job $i$.
...
& \text{Subject to} & & \sum_{i=1}^{n}x_{i,r}=cap_{r}, \quad && \forall r\in\{1,...,m\} \quad && \text{(resource capacity)} \\
& & & \sum_{r=1}^{m}x_{i,r}=dem_{i}, \quad && \forall i\in\{1,...,n\} \quad && \text{(job demand)} \\
& & & 0\le x_{i,r}\le1, \quad && \forall i, r
\end{aligned}
$$
...
| 100 | 92.5 | 0.00553 | 0.00557 | 1000 |
| 500 | N/A | 0.0377 | 0.0350 | 5000 |
| 1000 | N/A | 0.156 | 0.156 | 10 000 |

### 5.2 Solve Time vs. Problem Size

![Median solve time vs n_jobs](https://github.com/Joe-Bresee/linear-programming/raw/main/a1/scheduling-lp-benchmark/plots/time_vs_njobs_combined.png)
...
* **SciPy revised simplex does not scale.** From 0.014 s at $n=10$ it reaches 45.1 s at $n=50$ and 92.5 s at $n=100$ - a roughly 6,600× increase in time for a 10x increase in problem size.
* **HiGHS simplex and HiGHS IPM are nearly identical.** Both scale from ~0.002 s at $n=10$ to ~0.156 s at $n=1000$. This shows that the performance of the two HiGHS's algorithms depends on the HiGHS implementation optimizations and not their core algorithms.
* **The engineering gap is the dominant result.** At $n=50$ the ratio between SciPy revised-simplex and HiGHS simplex is $45.1 \text{ s} / 0.003 \text{ s} \approx 15,000\times$. At $n=100$ it is $92.5 \text{ s} / 0.006 \text{ s} \approx 16,700\times$. Both backends are simplex algorithms; the difference is entirely implementation.

## 6 Discussion

### 6.1 The Engineering Gap
The most noticeable result is just how slow the revised-simplex was. At $n=50$: 45.1 s vs. 0.003 s, a factor of roughly 15,000x between it and its HiGHS implementation. At $n=100$: 92.5 s vs. 0.006 s is a factor of ~16,700×. This gap is not evidence that the Simplex method is uncompetitive; it is evidence that the revised-simplex backend version is.

SciPy's revised-simplex is documented to be slow on problems with large numbers of equality constraints because it does not use the same optimization strategies as the two HiGHS do. Therefore the revised-simplex implementation should not be looked at as showing the simplex algorithm is weak, but as showing that unoptimized implementations of the simplex algorithm is weak - meaning that the real differentiator of slow performing Simplex solvers and fast performing ones is in how the Simplex method implementation is designed and programmed.

### 6.2 HiGHS Simplex vs. HiGHS IPM
Another finding I had is that HiGHS simplex and HiGHS IPM are almost indistinguishable on this problem across all (machine-allowing) tested sizes, with neither method consistently faster than the other. At $n=1000$ both reach ~0.156 s. Two different algorithms, a vertex-follower and an interior-point method, converge to almost the same performance on randomly generated scheduling instance problems. This is consistent with Spielman and Teng's smoothed analysis [3]: on practical instances with natural noise, meaning the worst-case runtime of Simplex is no longer important, and its replaced by its smoothed worst-case. The shared HiGHS implementation of both causes their runtimes to converge (under $n=1000$).
...
### 6.4 Limitations
This experiment uses a single LP structure (equality-constrained scheduling). Results may differ on inequality-dominated LPs, since the revised-simplex method from SciPy is known to have weak performance on equality constraints. Machine limits prevented SciPy revised-simplex from reaching $n=500$. Machine limits prevented runs on large $n=3000+$ to test HiGHS Simplex vs. HiGHS IPM at larger scale.

## 7 Conclusion

The experimental and theoretical evidence studied here refutes the conjecture as stated. The claim that "the Simplex Method is not a competitive option for solving LPs in practice" is refuted when the distinction between algorithmic class implementation is made.
...
[1] V. Klee and G. J. Minty, "How good is the simplex algorithm?" in *Inequalities III*, O. Shisha, Ed. New York: Academic Press, 1972, pp. 159-175. [Online]. Available: http://cgm.cs.mcgill.ca/~avis/Kyoto/courses/te/abs_examples.pdf

[2] N. Karmarkar, "A new polynomial-time algorithm for linear programming." *Combinatorica*, vol. 4, no. 4, pp. 373-395, 1984. [Online]. Available: https://www.stat.uchicago.edu/~lekheng/courses/302/classics/karmarkar.pdf

[3] D. A. Spielman and S.-H. Teng, "Smoothed analysis of algorithms: Why the simplex algorithm usually takes polynomial time," *J. ACM*, vol. 51, no. 3, pp. 385-463, 2004. [Online]. Available: https://arxiv.org/pdf/math/0212413

[4] Q. Huangfu and J. A. J. Hall, "Parallelizing the dual revised simplex method," *Math. Program. Comput.*, vol. 10, no. 1, pp. 119-142, 2018. [Online]. Available: https://arxiv.org/abs/1503.01889